...
(function() {
  ...
  // ===== PARSER (Claude) =====
  // Claude.ai has data-testid="user-message" for user turns
  // but NO data-testid for assistant turns.
  // Strategy: find conversation turn containers, check if they contain user-message
  function parseMessages() {
    var messages = [];

    // Find all user-message elements
    var userEls = document.querySelectorAll('[data-testid="user-message"]');
    if (userEls.length === 0) return fallbackParse();

    // Walk up from user messages to find the turn container level
    // Then collect all sibling turns at that level
    var turnLevel = findTurnLevel(userEls[0]);
    if (!turnLevel) return fallbackParse();

    var turns = Array.from(turnLevel.children).filter(function(el) {
      // Include if it contains a user-message
      if (el.querySelector('[data-testid="user-message"]')) return true;
      // Include if it has substantial text (likely assistant response)
    ...
    turns.forEach(function(turn) {
      var text = (turn.innerText || "").trim();
      if (text.length < 3) return;
      var hasUser = turn.querySelector('[data-testid="user-message"]') !== null;
      // Find the actual content element
      var contentEl = hasUser
        ? turn.querySelector('[data-testid="user-message"]')
        : findAssistantContent(turn);

      messages.push({
        id: "msg-" + idx,
        role: hasUser ? "user" : "assistant",
        text: (contentEl ? contentEl.innerText : text).trim(),
        element: turn,
        contentEl: contentEl || turn,
      });
      idx++;
    });
  ...
  // Walk up from a user-message element to find the parent that holds all turns
  function findTurnLevel(userEl) {
    var totalUserMsgs = document.querySelectorAll('[data-testid="user-message"]').length;
    var el = userEl;
    var depth = 0;
    while (el && el.parentElement && depth < 15) {
      var parent = el.parentElement;
      var siblings = Array.from(parent.children).filter(function(c) {
        return (c.innerText || "").trim().length > 3;
      });
      // Count how many siblings contain a user-message
      var userCount = siblings.filter(function(s) {
        return s.querySelector('[data-testid="user-message"]') !== null ||
          s.matches('[data-testid="user-message"]');
      }).length;
      // Right level: contains ALL user messages AND has non-user siblings too
      if (userCount >= totalUserMsgs && siblings.length > userCount) {
        return parent;
      }
      el = parent;
      depth++;
  ...
  // Find the main content area within an assistant turn
  function findAssistantContent(turn) {
    // Look for markdown-rendered content
    var md = turn.querySelector('.grid-cols-1, [class*="markdown"], [class*="prose"], p');
    if (md && md.innerText && md.innerText.trim().length > 3) return md;
    // Fallback: largest text child
    var best = null, bestLen = 0;
    turn.querySelectorAll('div').forEach(function(d) {
      var len = (d.innerText || "").trim().length;
      if (len > bestLen) { bestLen = len; best = d; }
    });
    return best || turn;
  }

  // Fallback: alternate even/odd from main
  function fallbackParse() {
    var messages = [];
    var conv = document.querySelector('main');
    if (!conv) return messages;
    var children = [];
    // Try to find substantial content blocks
    conv.querySelectorAll('div').forEach(function(el) {
      if (el.children.length < 3 && (el.innerText || "").trim().length > 20) {
        children.push(el);
      }
    });
    // Deduplicate (only keep elements that aren't children of others in the list)
    var filtered = children.filter(function(el) {
      return !children.some(function(other) {
        return other !== el && other.contains(el);
      });
    });
    filtered.slice(0, 50).forEach(function(el, idx) {
      messages.push({ id: "msg-" + idx, role: idx % 2 === 0 ? "user" : "assistant", text: el.innerText.trim(), element: el, contentEl: el });
    });
    return messages;
  }
